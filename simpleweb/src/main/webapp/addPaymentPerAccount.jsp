...
<body>
	...
	IBAN:
	<%=acc.getIBAN()%><br> Nume:
	<%=acc.getCustomer().getName()%><br> Sold:
	<%=acc.getAmount()%>
	Valuta:
	<%=acc.getCurrency()%><br>
	<hr>
	<FORM action="transactions/add" method="post">
		Suma: <input type="text" name="amount" value="0"> <select
			name="accBeneficiar">
			<%
				int i = 0;
			...
			<option <%=i == 0 ? "selected='selected'" : ""%>
				value="<%=a.getId()%>"><%=a.getIBAN()%>/<%=a.getCustomer().getName()%>
				<%
					i++;
					}
				%>
			
		</select> <input type="hidden" name="accPlatitor" value="<%=saccid%>">
		<br>
		<%
			if (!accList.isEmpty()) {
		%>
		...
		%>
		<B>No available corresponding account</B><br> <a
